...
export const PUT: APIRoute = async ({ request, cookies, locals }) => {
  try {
    ...
    const body = await request.json() as { lang: string; ids: number[] };
    const { lang, ids } = body;
    ...
    for (let i = 0; i < ids.length; i++) {
      const existing = await db.select().from(casinoLocaleSettings)
        .where(and(eq(casinoLocaleSettings.casinoId, ids[i]), eq(casinoLocaleSettings.lang, lang)))
        .get();

      if (existing) {
        await db.update(casinoLocaleSettings).set({ sortOrder: i, updatedAt: new Date() }).where(eq(casinoLocaleSettings.id, existing.id)).run();
      } else {
        await db.insert(casinoLocaleSettings).values({ casinoId: ids[i], lang, sortOrder: i, isVisible: true, createdAt: new Date(), updatedAt: new Date() }).run();
      }
    }
