...
export const GET: APIRoute = async ({ url, locals }) => {
  try {
    ...
    const localeResults = await db.select({
      id: casinos.id,
      name: casinos.name,
      slug: casinos.slug,
    ...
    if (localeResults.length === 0) {
      const globalResults = await db.select().from(casinos).where(eq(casinos.isActive, true)).orderBy(asc(casinos.sortOrder)).all();
      return new Response(JSON.stringify({ casinos: globalResults }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }

    return new Response(JSON.stringify({ casinos: localeResults }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
