...
export const GET: APIRoute = async ({ url, cookies }) => {
  try {
    ...
    if (!pageSlug || !lang) {
      return new Response(
        JSON.stringify({ error: 'page_slug and lang query params are required' }),
...
export const PUT: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const { pageSlug, lang, title, metaTitle, metaDescription, content, faqData } = body as {
      pageSlug: string;
      lang: string;
      title?: string;
      metaTitle?: string;
    ...
    return new Response(
      JSON.stringify({ pageContent: updated }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Internal server error';
